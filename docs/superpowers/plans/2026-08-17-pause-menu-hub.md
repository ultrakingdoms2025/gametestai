...
13. **The hub's race row had no way to reach `RaceUI._openStop` (the STOP RACE sheet), so it was dead code.** F7 used to raise it; once F7 was removed (deviation 10) the row still ran `openPanel()` unconditionally, which mid-race shows the setup picker with a no-op START. `RaceUI` now subscribes `bus.on('race:quitRequest', () => this._openStop())`, mirroring `MinigameUI`'s existing `minigame:quitRequest` wiring, and `openPanel()` refuses outright while `this.race?.racing`. The `main.js` row relabels to **Quit race** / hints "Stop this race" while racing and emits `race:quitRequest` instead of calling `openPanel()`.
