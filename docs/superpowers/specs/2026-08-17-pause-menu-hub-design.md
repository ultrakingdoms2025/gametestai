...
**Date:** 2026-08-17
**Status:** Approved in brainstorming; spec review round 4
...
- Browser-driven fullscreen exit (Esc in some browsers exits fullscreen and pointer lock together): `fullscreenchange` unlocks keyboard; hub shows via lockchange as usual. The Fullscreen item is a **preference** (`fullscreenPreferred` is untouched by a browser-driven exit), so it may read "On" while windowed until the next resume re-enters fullscreen — accepted; the item hint says "applies when you resume".
