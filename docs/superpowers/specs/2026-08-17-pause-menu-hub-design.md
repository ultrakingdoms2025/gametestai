...
**Prerequisite — the overlay counter becomes the single contract.** Today `_overlayOpen/_overlayClose` (local closures inside `_wire()`, `HUD.js:~1197-1233`) are fed by `race:menu`, `hud:block`, `audio:menu`, `bug-report:*`, `character:*`, `inventory:*`, `keybinds:*`, `mount:menu:*` — but **not** by `help:open/close` (HelpMenu), `ui:modal` (MazeMap; nothing listens to it today) or `minigame:menu` (MinigameUI). Step one: promote `_overlayOpen`, `_overlayClose`, `_schedRelock` to prototype methods (testable), and subscribe the three missing events (`help:open/close`, `ui:modal {open}`, `minigame:menu {open}`) so every hub-reachable panel raises and lowers the count. Without this, `openFromHub` would re-show the hub on top of Help/Map/Minigame and never get a return event.
...
- **Esc from gameplay under keyboard lock**: capture-phase window `keydown` — when `code === 'Escape' && !e.repeat && input.locked && this._overlayCount === 0 && !this._chatOpen && !input.textCaptured` → `input.exitLock()`. (Without keyboard lock the browser already exits pointer lock on Esc; with it, Esc reaches the page and today does nothing. Because Help now counts as an overlay, Esc with Help open closes Help only.)
...
- `_overlayClose`: **behaviour change, declared** — `_schedRelock()` today runs on *every* close; it now runs only when the count reaches 0 and `_hubReturn` is false. When the count reaches 0 and `_hubReturn` is true → `_hubReturn = false; showPauseOverlay(true)`.
...
- `KeybindMenu.FIXED_KEYS`: `Esc — Pause menu`, `F1 — Help`, letter keys as today (add the missing `M`); **both** prose notes (`~:167` and the FIXED list header) read "Fixed keys (Esc, F1 and the letter keys J I B M K) cannot be rebound; F2–F12 are left to the browser."
...
- Help open during locked gameplay + Esc: Help counts as an overlay (prerequisite), so HUD's Esc handler is inert and Help's own Esc closes it; no hub.
...
- HUD flow (`Object.create(HUD.prototype)` + minimal fields, `showPauseOverlay`/`_schedRelock` stubbed — possible because the prerequisite promotes them to methods): `_overlayClose` to 0 with `_hubReturn` → re-show, no relock; without → relock; `openFromHub` whose `run` opens nothing → re-show.
- **Hand-off test with a fake bus** (the one that would catch a panel outside the counter): a real `HUD` cannot be built headlessly, so a stub with the real `_overlayOpen/_overlayClose/openFromHub/_wireOverlayEvents` methods and a tiny EventBus replays each hub-reachable panel's *actual* open/close event pair (`character:open/close`, `mount:menu:open/close`, `inventory:open/close`, `hud:block`, `ui:modal`, `race:menu`, `minigame:menu`, `help:open/close`, `audio:menu`, `keybinds:open/close`, `bug-report:open/close`) and asserts: after `openFromHub` + open event the overlay is hidden and the count is 1; after the close event the overlay is re-shown and `_schedRelock` was not called.
- Source-text guards: (a) `PAUSE_MENU_IDS` ⊆ ids present in `main.js`'s item list (regex `id: '<id>'`); (b) no `code === 'F<n>'`, `['F<n>'`/`'F<n>',` list entries, or `pressed('F<n>')` for n ∈ 2..12 remain in `src/` outside the exported `RESERVED_CODES` literal in `Input.js` and comments; (c) none of the dead-key label strings from §4.6 remain (`grep -n "F2 close\|F10 close\|F4 or Esc\|F7\b\|F8 to quit\|F9 / F12\|<i>F3</i>" src/ui`).
