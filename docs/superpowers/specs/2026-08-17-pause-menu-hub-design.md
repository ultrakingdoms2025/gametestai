...
- `openFromHub(run, { overlay = true } = {})`: with `overlay:false` (Help) just `run()`; otherwise `_hubReturn = true; showPauseOverlay(false); run();` then, if `_overlays.size === 0` (the panel refused/opened nothing) → `_hubReturn = false; showPauseOverlay(true)`.
- `_overlayClose(id)`: **behaviour change, declared** — `_schedRelock()` today runs on *every* close; it now runs only when the Set becomes empty and `_hubReturn` is false, together with today's `showPauseOverlay(false)` (which survives: it hides a pause overlay that fired while a blocking UI was open). When the Set becomes empty and `_hubReturn` is true → `_hubReturn = false; showPauseOverlay(true)`.
...
- **Fullscreen preference.** `Input.requestLock()` unconditionally requests fullscreen today (`Input.js:168-176`), which would undo a "Fullscreen: Off" the moment the player resumes. Add `input.fullscreenPreferred` (default `true`, persisted in `localStorage['aether:fullscreen']`); `requestLock()` requests fullscreen only when it is true; the hub item flips it and calls `requestFullscreen`/`exitFullscreen` accordingly. Label reads the preference, not the DOM (`requestFullscreen` resolves asynchronously): `Fullscreen: ${input.fullscreenPreferred ? 'On' : 'Off'}`. **Trade-off, disclosed in the item hint**: `navigator.keyboard.lock()` only works in fullscreen, so "Off" hands Ctrl+W (crouch-walk!) back to the browser — hint: "Off gives Ctrl+W back to the browser; the save prompt still guards it" (`SaveGame`'s `beforeunload` remains).
