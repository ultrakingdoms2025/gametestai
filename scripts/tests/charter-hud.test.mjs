...
test('no class this panel invents already means something else', async () => {
  ...
  /* NORMALISE FIRST. This repository checks out with `core.autocrlf` on, so
   * `HUD.js` has CRLF here and LF in the worktree it was written in. The
   * anchors below look for `\n  }\n`; against CRLF that matches nothing,
   * `indexOf` returns -1, `slice(from, -1)` runs to the end of the file, and
   * every later method's classes fall into `mine` AND into `shared` - so `own`
   * comes out empty and the gate reports that its own scrape has broken.
   *
   * Green in the worktree, red in the checkout, and about the wrong thing in
   * both. This is the THIRD time this exact defect has been paid for here: see
   * the roadmap's per-phase discipline note, and the commit "The marketplace
   * scrape was green in a worktree and red in the checkout", which fixed it in
   * `marketplace-offline.test.mjs` the same way. Any textual scrape in this
   * repository must not care how the file was checked out. */
  const hudSrc = (await readFile(new URL('HUD.js', uiDir), 'utf8')).replace(/\r\n/g, '\n');
