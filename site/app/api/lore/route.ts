 ...
 *
 * Not a query parameter, and not a body field. `currentServer` re-checks
 ...
 * relies on for the quest catalogue.
 *
 ...
 */
async function loreScope(): Promise<string | null> {
  /* Null on ANY failure, which is the safe direction and the same one
   * `currentServer` documents for itself: null is the platform partition, so a
   * session or database hiccup shows a player the default lore rather than
  ...
  try {
    ...
    return await currentServer(playerId);
  } catch (error) {
...
export async function GET() {
  try {
    const serverId = await loreScope();
    const rows = await getLoreEntries(serverId);
     ...
     * The merge rule lives in the ORDER BY, not here. */
    ...
    return NextResponse.json({ entries, server_id: serverId });
  } catch (error) {
