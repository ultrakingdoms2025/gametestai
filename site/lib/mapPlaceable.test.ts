 ...
 * rows from `buildMarketplaceSeedItems`, and the game refused all nine with `reason: 'item'` while the editor
 * had offered them beside the ammo packs with nothing to tell them apart. The consumables are asserted
 * placeable THROUGH their world-stamped key (`spell_velocity_25:station`), the shape the API hands the
 * editor, because the game resolves those by key and not by effect; a rule that read only the effect would
 * hide fifteen items the game can place.
 */
...
describe('placeableReason', () => {
  ...
  it('a mount power is applied to the rider: the nine rows the admin placed on station, all refused by the game', () => {
    ...
    for (const s of nine) {
      ...
      expect(placeableReason({ source_key: s.source_key, config: s.action_config }), s.name).toBe('mount powers are applied to the rider, not placed in a world');
    }
  });
...
describe('partitionPlaceable and hiddenItemsText', () => {
  ...
  it('keeps the placeable rows in their order, by identity, and the hidden ones with their reasons', () => {
    ...
    expect(placeable).toEqual([items[0], items[3]]);
    expect(placeable[0]).toBe(items[0]);
    expect(hidden).toEqual([
      { item: items[1], reason: 'mount powers are applied to the rider, not placed in a world' },
      { item: items[2], reason: 'cosmetics unlock in the wardrobe; they cannot lie on the ground' },
      { item: items[4], reason: NOT_A_PICKUP_TEXT },
      { item: items[5], reason: 'mount powers are applied to the rider, not placed in a world' },
    ]);
  });
  ...
  it('the line under the list counts the hidden rows and names their kinds, in a fixed order', () => {
    ...
    expect(hiddenItemsText(hidden)).toBe('4 items cannot be placed in a world (mount powers, cosmetics, heals) — they are granted by purchase');
    expect(hiddenItemsText(hidden.slice(0, 1))).toBe('1 item cannot be placed in a world (mount powers) — it is granted by purchase');
    expect(hiddenItemsText(partitionPlaceable([items[4], { ...items[4], id: '7', action_config: {} }]).hidden))
      .toBe('2 items cannot be placed in a world (heals, other items) — they are granted by purchase');
    expect(hiddenItemsText([])).toBe('');
  });

  it('on the real seed, every mount power and cosmetic is hidden and every ammo pack, part, skin and spell is offered', () => {
    ...
    expect(effectsOf(hidden.map((h) => h.item))).toEqual(new Set(['grant_mount_power', 'unlock_cosmetic', 'restore_health']));
    expect([...effectsOf(placeable)].sort()).toEqual(['grant_ammo', 'grant_item', 'loot_magnet', 'modify_firepower', 'modify_speed', 'pause_npcs', 'portal_ping', 'shield']);
