...
/**
 * The art pipeline's SERVER HALF: anything that renders, fetches or re-encodes.
 *
 * ── Why this is a separate file ───────────────────────────────────────────
 *
 * These functions reach for `sharp` and the `ai` SDK. `marketplaceImages.ts` is
 * imported by `components/MarketplaceAdminPanel.tsx`, which is a CLIENT
 * component - so Turbopack traced `sharp` into the browser bundle and the
 * production build failed outright:
 *
 *   the chunking context (unknown) does not support external modules
 *   (request: node:child_process)
 *   ./site/node_modules/sharp/dist/index.mjs [Client Component Browser]
 *   ./site/lib/marketplaceImages.ts [Client Component Browser]
 *   ./site/components/MarketplaceAdminPanel.tsx
 *
 * THREE DEPLOYS FAILED THAT WAY before anybody noticed, because a failed
 * deployment leaves the previous one serving: the site kept working, the fixes
 * simply never arrived, and the symptom was a player reporting that nothing had
 * changed. A dynamic `await import()` inside a `try` does not help - the tracer
 * follows it regardless.
 *
 * So the split is by RUNTIME, not by topic: `marketplaceImages.ts` keeps the
 * pure string work any environment can run (prompts, URLs, recognising a
 * recipe), and everything needing Node lives here behind `server-only`, which
 * The boundary is by convention rather than enforced: `server-only` is not a
 * dependency here, and adding one to police an import felt like the wrong trade
 * at the moment production was red. If this breaks again, that package is the
 * fix - it turns a client import into a loud error naming this file instead of
 * an opaque chunking failure three files away.
 */
